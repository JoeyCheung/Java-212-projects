...
public class Project4 {
	...
	public static void main (String[] args) {
		...
		*/
		...
		HashMap<Box, Integer> unsortedBoxList = new HashMap<Box,Integer> (); //puts the unsorted list on the left side
		TreeMap<Box, Integer> sortedBoxList = new TreeMap<Box,Integer> (); //puts the sorted list on the right side
		...
		while (line != null) {
			...
			unsortedBoxList.put(box, box.volume()); //this is used now to add the box to the left side of the list
			sortedBoxList.put(box, box.volume()); //this is used now to add the box to the right side of the list
			line = file.readLine(); //goes to next line
		}
		BoxGUI gui = new BoxGUI(unsortedBoxList,sortedBoxList); //instantiates GUI
	}
